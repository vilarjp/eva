...
**Twelve composable, scope-adaptive Claude Code skills for compound engineering —
reverse-engineer, plan, diagnose, audit, execute, review, triage PR feedback, ship, remember, and a routing brain that picks the right one for any input.**

[![License: MIT](https://img.shields.io/badge/license-MIT-blue)](LICENSE)
[![Version 0.12.0](https://img.shields.io/badge/version-0.12.0-brightgreen)](.claude-plugin/plugin.json)
...
A Claude Code plugin with twelve sharp, composable skills:

- **`eva`** — the plugin's routing brain. Takes any input — a rough goal, a bug description, a Jira card, a GitHub PR, a file path, or *"what should I do with this?"* — and returns an ordered **Routing Plan**: which EVA skill(s) to run, in order, with a complexity tier (Lightweight / Standard / Deep) and a one-line rationale per step. Classifies intent into one of eleven tags (`understand` / `plan-feature` / `design-feature` / `revise-pair` / `fix-bug` / `refactor-debt` / `implement-approved-plan` / `review-diff` / `handle-pr-feedback` / `ship-diff` / `capture-learnings`), trims the pipeline by reading the adjacent `docs/<DATE>-<slug>/` folder (skip `/prd` if `PRD.md` is already approved, skip `/diagnosis` if `DIAGNOSIS.md` is, etc.), and marks `/solutions` optional on Lightweight work. Default mode is **RECOMMEND** (emit the plan and stop); opt-in **DRIVE** mode (triggered by *"do it"* / *"follow the pipeline"* / *"just run it"*) dispatches ONLY the first skill — subsequent steps are always an explicit user call because every downstream skill has its own approval gate. Fetches Jira cards (Atlassian MCP with `fields: ["*all"]` + the full `expand` set), GitHub PRs/issues (via `gh`), and local files/folders (Read/Glob/Bash) when the input demands classification — but never drafts artifact content, never edits code, never commits, never posts PR comments, never transitions Jira state. Pure router — substitutes for no skill, honours every downstream gate.
...
- **`audit`** — survey stable code for structural debt on a static snapshot (no git history, no blame, no runtime tracing). Catalogues findings against a three-layer smell vocabulary — the 13 Fowler smells (Feature Envy, Primitive Obsession, Shotgun Surgery, Data Clumps, Divergent Change, Long Parameter List, Message Chain, Middle Man, Refused Bequest, Speculative Generality, Temporary Field, Duplicate Code, Comments as Deodorant), the 7 no-workaround categories (TYPE / LINT / SWALLOW / TIMING / PATCH / SCATTER / CLONE) reframed as audit postures, and 5 test anti-patterns (mock-behaviour assertion, test-only production methods, mocking without understanding, incomplete mocks, tests-as-afterthought). Every finding is severity-ranked (P0 Blocker debt / P1 Major / P2 Minor / P3 Nit) with a cost band, carries verbatim evidence, a named smell, and a handoff table that `/execute` REFACTOR mode consumes directly. Real current bugs leave the audit and route to `/diagnosis` via a separate **Bugs surfaced** section — the audit never masquerades as a bug tracker. Pure reporter — never edits code, never commits, never chains to another skill; writes only `AUDIT.md` after a HARD GATE with explicit user approval.
- **`execute`** — implement a feature from an approved SPEC/REVISION, fix a bug from an approved DIAGNOSIS, address selected findings from an approved CODE-REVIEW (FIX mode), apply selected refactors from an approved AUDIT (REFACTOR mode), or execute a direct human prompt when no artifact exists; follows a red-green-refactor TDD loop with a narrow Verification Mode escape hatch for infra/config/docs, enforces minimal-diff and NOTICED-BUT-NOT-TOUCHING scope discipline, commits per vertical-slice on a non-protected branch, keeps its codebase pre-scan internal, and produces a durable EXECUTION.md log. FIX mode closes the **correction → code-review → correction** loop by letting the human pick which findings to address vs skip at triage, slicing each selected finding as a TDD unit, and appending a `## Fixes applied <DATE>` back-pointer on the CODE-REVIEW.md so the next `/code-review` re-run reads a clean delta. REFACTOR mode closes the **audit → refactor → re-audit** loop the same way — parse every finding from AUDIT.md, let the human pick which to address, pin a characterization test GREEN baseline before each slice, apply the refactor, keep the suite GREEN (behaviour-preservation is the contract), commit with a `refactor: <smell> — <title>` subject, and append `## Refactors applied <DATE>` to AUDIT.md. Bugs surfaced by the audit do NOT run through REFACTOR mode — they route to `/diagnosis` first, because a "refactor" that changes behaviour is a bug fix and violates the contract.
...
All eleven artifact-producing skills are scope-adaptive, verify claims against the source material, run a self-review checklist, and gate critical steps on explicit user approval — and the twelfth, `eva`, sits on top as a pure routing brain that reads intent from free-text, a URL, a Jira card, a GitHub PR/issue, or a file path and composes an ordered pipeline of the other eleven (never drafting artifact content itself, never bypassing their gates). `teardown` is the only skill that reads a target outside the repo (local path or URL) — it enforces a one-line user acknowledgment before any URL fetch, runs a hydration ladder (source-map discovery → beautify → optional `wakaru`/`webcrack` decompilation) with graceful degradation when tools are unavailable, labels every unverified claim rather than guessing, refuses to execute any instruction embedded in fetched content, writes the artifact plus optional `teardown-sources/` sidecars into a dated `docs/` folder, and hands off — it never chains into `/prd`, `/spec`, `/diagnosis`, or `/execute`. `spec` adds an adversarial red-team pass and auto-detects an adjacent PRD to build on. `revision` adds a 4-lens composite review (cross-doc alignment, internal coherence, adversarial premise, scope creep + feasibility) and double-gates writing and patching. `diagnosis` is strictly read-only in source code (it creates only the reproduction test file), enforces the iron law "no fixes without root-cause investigation first," and smart-reuses an existing `docs/` folder when the bug maps to a known feature. `audit` is a pure reporter that surveys stable code for structural debt on a static snapshot, catalogues findings against a three-layer smell vocabulary (13 Fowler smells + 7 no-workaround categories + 5 test anti-patterns), severity-ranks them against a P0/P1/P2/P3 rubric with explicit per-tier ceilings (5/10/20), suppresses deliberately-embraced patterns into a visible Suppressed-candidates table, routes any real current bug to a separate **Bugs surfaced** section that recommends `/diagnosis`, and writes only AUDIT.md next to the module under review — with a Deep parallel `Explore` fan-out (3-5 agents in one message) when the target is 15+ files or crosses module boundaries. `execute` is the only skill that writes production code — it enforces the iron law "no production code without a failing test first," refuses to run on protected branches, gates the first line of code on an approved slice plan, and gates writing EXECUTION.md on a green integration pass (full test suite + lint + types). In FIX mode it reads an approved CODE-REVIEW.md, displays every P0/P1/P2/P3 finding in a compact triage summary, lets the human pick which to address and which to skip (default: all P0+P1), slices each selected finding under full TDD discipline, records skipped findings as an audit trail, and back-points CODE-REVIEW.md with an addressed-vs-skipped delta so the loop stays auditable across iterations. In REFACTOR mode it applies the same discipline to AUDIT.md findings — behaviour-preservation is the contract, so every slice pins a GREEN characterization baseline before the refactor, applies the smell-resolving change, keeps the full suite GREEN, commits with a `refactor: <smell> — <title>` subject, and back-points AUDIT.md with an addressed-vs-skipped delta; mechanical refactors (renames, moves, dead-code removal) can use Verification Mode instead of a characterization test, and any "bug surfaced" item in AUDIT.md is explicitly diverted to `/diagnosis` rather than refactored. `code-review` is a pure reporter that reads the current diff, fans out seven specialized reviewers (plan-alignment, code-quality, convention, test, security, performance, adversarial) through a two- or three-stage pipeline with dedup + confidence-gated suppression + cross-reviewer agreement boost, smart-reuses an adjacent spec folder when the diff maps to a known feature, and writes only CODE-REVIEW.md — never touches production code, tests, commits, or pushes. `pr-feedback` is the inbound counterpart that closes the external-review loop — it takes a GitHub PR reference, enforces a mandatory branch-checkout gate (offers `gh pr checkout` first, refuses-until-correct with manual commands on failure), unions every non-dismissed reviewer's inline thread comments (via GraphQL so `isResolved` / `isOutdated` / thread replies are first-class), timeline comments, and top-level review bodies, dedupes by `thread_id`, classifies each comment into five author-facing verdicts (must-fix / nice-to-have / question / push-back / already-done) with a 0-100 confidence score derived from Conventional Comments labels + decorators + spec cross-reference + anchor freshness + CODE-REVIEW.md fingerprint agreement, drafts a copy-pasteable reply per verdict, and hands off must-fix items to `/execute` FIX mode via a structured Handoff table — it is pure triage and never posts to the PR, never edits source, never commits. `commit-push` is the counterpart that ships the diff — a scope-adaptive pre-commit gate (secrets + protected-branch on Lightweight, + tests + lint + debug-artifacts on Standard, + scope-vs-diff + test-coverage-for-code-changes + chore-exemption audit on Deep), a hard human-gated branch confirmation in every run, a human-approved logical split of up to three Conventional-Commits-style messages drafted to match the repo's detected convention, explicit per-file staging, a fetch-and-rebase before push, and an optional back-pointer to an adjacent EXECUTION.md / DIAGNOSIS.md — but refuses protected branches, refuses to open PRs (`gh pr create` is out of scope), refuses `--force` / `--no-verify` / `git add -A`, and refuses to commit secrets regardless of what the user insists. `solutions` is the pipeline's memory terminator — it reads every artifact in the folder (PRD, SPEC, REVISION, DIAGNOSIS, EXECUTION, CODE-REVIEW) and distils durable, file-independent learnings into a scope-inherited `SOLUTIONS.md` with compact mini-ADRs, Gotchas, What Didn't Work, and (for bugs) Prevention. It is a pure synthesiser: never edits code, never touches `CLAUDE.md` / `AGENTS.md`, never chains to another skill — it enforces the Iron Law of durability (behaviour invariants over file:line) and appends on re-runs instead of overwriting, so a folder's SOLUTIONS.md becomes the accretive knowledge record a cold-context future session actually reads before touching that area again.

## What they do

### `eva`

Produces a **Routing Plan** — inline, no `docs/` artifact. The plan has:

- **Understood** — one-sentence paraphrase of the user's goal, the input type (free-text / GitHub PR / GitHub issue / Jira card / local file / local directory / `docs` folder), and 1-2 lines of evidence cited from the actual reads.
- **Recommendation** — a complexity tier (Lightweight / Standard / Deep) and an ordered pipeline of 1-6 EVA skills, each with a one-line rationale.
- **Skipped steps** — which EVA skills were explicitly NOT included, and why (e.g. *"/prd — PRD.md already approved in the folder"*).
- **Alternatives considered** — a real alternative pipeline with one-line reason why it lost.
- **Next step** — how to proceed: say *"do it"* to have `/eva` dispatch the first skill, or invoke the first skill directly.

The flow is a fixed 7-phase pipeline rather than scope-adaptive: Pre-flight (parse args, detect DRIVE phrase) → Phase 1 Classification reading (the minimum needed — `gh pr view --json`, `gh issue view --json`, Atlassian MCP `getJiraIssue` with `fields: ["*all"]` + the full `expand` set, `ls -la` + head + `git status` on paths, `find | wc -l` on directories, nothing for pure free-text) → Phase 2 Intent classification (pick exactly one tag from an 11-row table) → Phase 3 Complexity assessment (file count, issue labels, scope keywords → one tier) → Phase 4 Pipeline composition (map intent → canonical pipeline, then trim based on artifacts already present in the target folder) → Phase 5 Self-review checklist → Phase 6 Emit the Routing Plan (under 30 lines, one-line rationales) → Phase 7 Optional DRIVE dispatch.

The skill is **strictly read-only** and **pure-router** — it never writes `.md` files, never edits code, never commits, never pushes, never posts PR comments, never transitions Jira issue state, and never drafts artifact content (no PRD body, no SPEC section, no diagnosis hypothesis, no audit finding, no code-review comment, no commit message). DRIVE mode dispatches **only the FIRST skill** in the pipeline — subsequent steps are always an explicit user call, because every downstream skill has its own HARD GATE. `/eva` does not pre-approve downstream artifacts, does not override named skills (if the user typed `/audit`, the router stays out of the way), and does not substitute for the skill it routes to (a recommendation with *"and here's the root cause I found"* is a protocol violation — the root cause belongs in `DIAGNOSIS.md`, produced by `/diagnosis`).
...
Produces `docs/YYYY-MM-DD-<slug>/AUDIT.md` (or reuses an adjacent feature folder when the audited module maps to a known feature), containing:

- **TL;DR posture** — `clean` / `healthy with nits` / `debt-heavy` / `rewrite-candidate`; counts per severity (P0 / P1 / P2 / P3); scope tier; one-line verdict
- **Scope of audit** — target paths, LOC, file count, scope tier, whether Deep parallel `Explore` fan-out ran, consumer-surface sweep status, exemplar file chosen for calibration
- **Findings grouped by severity** (P0 Blocker debt / P1 Major / P2 Minor / P3 Nit) — each with:
  - Named smell from the three-layer vocabulary (Fowler / no-workaround category / test anti-pattern)
  - File:line anchor and verbatim evidence (5-30 word quote from the code)
  - Why it matters — cost band (incident risk, change-blocker, maintenance drag, cosmetic)
  - Recognition test — the one-sentence heuristic that identifies this smell in the future
  - Suggested refactor direction (minimal, no new scope — REFACTOR mode's starting point)
  - `seen_in` cross-file cluster when the same smell recurs (Shotgun Surgery indicator)
- **Architecture observations** — cross-cutting shape findings that don't fit a single file:line (layering violations, cohesion drift, coupling amplifiers, boundary leaks)
- **Positives** — what the module does well, so REFACTOR mode doesn't regress it
- **Suppressed candidates** — smells that matched a pattern but were suppressed because the repo deliberately embraces them (ADR cited, comment confirmed, or convention file named); visible, never silently dropped
- **Bugs surfaced** — any finding that turned out to be a live crash / data loss / silent-failure / security-hole — routed to `/diagnosis`, NOT addressable in REFACTOR mode because a behaviour-changing fix violates the preservation contract
- **Handoff table** — one row per P0/P1/P2/P3 finding (ID, severity, smell, file:line, suggested direction) that `/execute` REFACTOR mode parses directly at Phase 0.5 triage
- **Scope calibration footer** — P1 ceiling per tier (5 / 10 / 20), actual count, and a re-calibration flag when ceilings are exceeded (signals P2s mis-tiered as P1)

The flow is **scope-adaptive**: a Lightweight target (1-3 files, <500 LOC, one cohesive module) gets a compact artifact with 0-1 questions, no fan-out, and a P1 ceiling of 5; a Standard target (4-15 files, 500-3000 LOC, one feature or bounded module) gets a pattern-exemplar sweep + required architecture observations + a P1 ceiling of 10; a Deep target (15+ files, 3000+ LOC, cross-module, shared kernel, concurrency, public API surface, or "audit the whole service") gets the full ceremony — 3-5 parallel `Explore` agents dispatched in a single message (one per submodule + consumer surfaces + shared utilities), mandatory consumer-surface sweep, Hyrum's-Law review, and a P1 ceiling of 20. Risk-tag promotion lifts the tier one step when the target touches concurrency, auth, payments, migrations, or public APIs regardless of size.

The skill is a **pure reporter** — it never writes code, never edits the audited module, never commits, never chains to another skill. Writing `AUDIT.md` is gated twice: a self-review checklist (severity calibration matches the rubric, every finding quotes verbatim evidence, deliberate patterns are suppressed not flagged, bugs are routed to `/diagnosis` not mixed into refactor findings, clean passes are allowed) and a HARD GATE that blocks writing until the user approves the finding set, the scope tier, and the output path. Clean passes are valid — a module with zero P0/P1 findings produces an artifact with Positives populated and the "surface is stable" posture, not a padded list of nits. The hand-off to REFACTOR mode is explicit — the user runs `/execute` on the AUDIT.md in a separate turn; a silent chain would bypass the human's findings-triage decision and the behaviour-preservation contract.
...
- **Findings Addressed** (FIX mode) — table of `F-N | severity | title | file:line | slice | commit | test` for every finding the user selected at Phase 0.5 triage
- **Findings Skipped** (FIX mode) — table of `F-N | severity | title | reason` for every finding the user chose to defer; these persist as open issues for the next re-review
- **Findings Addressed** (REFACTOR mode) — table of `F-N | severity | smell | title | file:line | slice | commit | test` where `test` cites the characterization test that stayed GREEN through the refactor (or `verification` for mechanical slices)
- **Findings Skipped** (REFACTOR mode) — table of `F-N | severity | smell | title | reason` for every finding the user chose to defer; these persist as open debt items for the next re-audit
- **Bugs surfaced routed to /diagnosis** (REFACTOR mode, when AUDIT.md had a Bugs surfaced section) — table of audit bug IDs with shape, file:line, and a `/diagnosis` recommendation; never processed by REFACTOR mode
- **Slice table** — one row per vertical tracer-bullet slice (intent, mode `full-tdd | characterization-tdd | verification`, test file, commit SHA, result)
- **Per-slice detail** with literal RED proof (or characterization GREEN baseline in REFACTOR mode), GREEN proof, files touched, commit SHA, acceptance criteria satisfied (Verification-Mode slices record a build + suite proof instead of RED)
- **Integration Gate** — literal output from a fresh full test suite, lint, and type-check; skipped-tests audit; regression red-green proof (BUG mode and FIX mode when practical — one proof per addressed P0 finding at minimum in FIX mode); behaviour-preservation proof (REFACTOR mode — full suite GREEN before Slice 1 and after the last slice)
- **Acceptance Criteria Trace** — table mapping every SPEC criterion / DIAGNOSIS reproduction test / selected CODE-REVIEW finding / selected AUDIT finding to the slice and test (or characterization test in REFACTOR mode) that satisfies it
...
- **Relationship to Source Artifact** — FEATURE/BUG/FIX/REFACTOR/RAW disposition plus the back-pointer appended to SPEC.md (`## Implemented <DATE>`), DIAGNOSIS.md (`## Fixed <DATE>`), CODE-REVIEW.md (`## Fixes applied <DATE>` with addressed-vs-skipped tables), or AUDIT.md (`## Refactors applied <DATE>` with addressed-vs-skipped tables including the named smell per finding, and a separate "bugs surfaced routed to /diagnosis" block)

The flow is **scope-adaptive**: a Lightweight slice (typo, rename, one-liner) gets 1-2 slices with 0-1 questions; a Standard feature or bug (3-10 files, clear SPEC phases or named root cause) gets 2-4 slices with per-slice RED-GREEN-REFACTOR; a Deep change (10+ files, concurrency/migration/integration) gets 2-4 slices plus a risk-mitigation mapping and mandatory regression proof in BUG mode; a REFACTOR pass scales the same way against AUDIT findings, with characterization tests capturing the GREEN baseline before each slice.

The skill **auto-detects the source of truth** in priority order: explicit path (filename routes to mode — `AUDIT.md → REFACTOR`, `CODE-REVIEW.md → FIX`, `SPEC.md → FEATURE`, etc.) > REFACTOR-mode prompt triggers ("apply the refactors", "resolve the P1 smells", "apply the audit") matched against the newest `AUDIT.md` with `status: approved` > FIX-mode prompt triggers ("address findings", "apply the code-review", "fix the blockers") matched against the newest `CODE-REVIEW.md` with `status: approved` > `REVISION.md` with `patches_applied: true` > `SPEC.md` with `status: approved` > `DIAGNOSIS.md` with `status: approved` > `PRD.md` alone (warns to run `/spec` first) > raw prompt (RAW mode). When the detected primary artifact sits in a folder that also has an approved CODE-REVIEW.md (FIX mode) or AUDIT.md (REFACTOR mode) with open findings, the skill offers a one-question mode-switch before advancing. Execute is the only eva skill that **writes production code**, and it is gated twice: HARD GATE 1 blocks the first line of code on approved slice plan + path; HARD GATE 2 blocks writing EXECUTION.md on a fully green integration pass (plus behaviour-preservation proof in REFACTOR mode). A protected-branch check at Pre-flight refuses `main`/`master`/`production`/`prod`/`stable`/`live`/`trunk`/`release*` before any work happens.
...
In **REFACTOR mode**, the same Phase 0.5 triage runs against AUDIT.md — the parser reads every P0/P1/P2/P3 finding in the Handoff table, subtracts any finding already resolved by a prior `## Refactors applied <DATE>` section, separates **Bugs surfaced** items out of the refactor pool entirely and records them as a dedicated "routed to /diagnosis" block (a behaviour-changing fix cannot run through REFACTOR mode — it violates the preservation contract), and presents the same A/B/C/D selection question. The critical difference from FIX mode is the per-slice discipline: instead of RED-first TDD, each slice is **characterization-tdd** — pin the current behaviour with a GREEN characterization test BEFORE touching the code, apply the refactor, re-run the same test, verify it stays GREEN, then run the full suite to confirm no collateral regressions. Mechanical refactors that provably don't change behaviour (pure renames, file moves, dead-code removal, signature-preserving extractions the type-checker verifies) may use **Verification Mode** instead — a captured full-build + full-suite green proof substitutes for the characterization test. Slice intent is named with the smell (`Refactor F-3 (P1, Shotgun Surgery): extract currency-conversion module`), commits use a `refactor: <smell> — <title>` subject referencing the finding ID in the body, and the EXECUTION.md lists every addressed finding with its named smell alongside a skipped-findings audit trail. Back-pointer is `## Refactors applied <DATE>` appended to AUDIT.md (with addressed + skipped tables, same shape as FIX mode's CODE-REVIEW back-pointer), so the next `/audit` re-run reads a clean delta. The behaviour-preservation proof — full suite GREEN before Slice 1 and after the last slice — is captured in the Integration Gate log as a literal before/after comparison, not an assertion.
...
## Invariants (all twelve skills)

`eva` (the routing brain) shares only the first four bullets below — it does not write an artifact, so the clarifying-question cap, HARD GATE, and source-verification rules apply only to the eleven artifact-producing skills.
...
Additional invariants for `audit`:

- **Pure reporter.** The only file the skill creates is `AUDIT.md`. Never edits, stages, commits, or pushes the audited code. Never applies a refactor — handoff to REFACTOR mode is a separate explicit human turn.
- **Static snapshot only.** No git history, no blame, no runtime tracing, no profiling. The audit reads the code as it is today and catalogues structural debt against a fixed smell vocabulary — temporal reasoning ("this was added three months ago") is out of scope because it leads to blame framing.
- **Three-layer smell vocabulary, no free-form judgment.** Every finding names its smell from one of three layers: the 13 Fowler smells (Feature Envy / Primitive Obsession / Shotgun Surgery / Data Clumps / Divergent Change / Long Parameter List / Message Chain / Middle Man / Refused Bequest / Speculative Generality / Temporary Field / Duplicate Code / Comments as Deodorant), the 7 no-workaround categories (TYPE / LINT / SWALLOW / TIMING / PATCH / SCATTER / CLONE), or the 5 test anti-patterns (T1 mock-behaviour assertion / T2 test-only production methods / T3 mocking without understanding / T4 incomplete mocks / T5 tests-as-afterthought). A finding without a named smell is a narrative, not an audit row. See `references/smell-playbook.md`.
- **Deliberate patterns are not smells.** If a finding's alleged smell is called out in an ADR, a code comment, a convention file (CLAUDE.md / AGENTS.md / CONTRIBUTING.md), or visibly embraced by the repo's wider pattern (DAMP tests, flat module structure, explicit factories), suppress it and list it under **Suppressed candidates** with the reason — never flag it as debt.
- **Real current bugs leave the audit.** A finding that is actually a live crash / data loss / silent failure / security hole moves to the **Bugs surfaced** section and recommends `/diagnosis`. The audit never masquerades as a bug tracker, and REFACTOR mode never runs on a bug (a behaviour-changing fix violates the preservation contract).
- **Severity calibration is load-bearing.** P0 = "we will page on this within a month"; P1 = "the next change to this area will hit this"; P2 = "annoying to read, no active risk"; P3 = "worth five minutes, otherwise drop." Per-tier P1 ceilings (Lightweight 5, Standard 10, Deep 20) are re-calibration signals — exceeding them almost always means P2s mis-tiered as P1. An audit where everything is P1 is noise.
- **Clean passes are valid.** An audit with zero P0/P1 findings produces an artifact with Positives populated and a "surface is stable" posture — padded nits are worse than an honest clean pass.
- **Deep mandates parallel fan-out.** On Deep scope, Phase 1 dispatches 3-5 `Explore` agents in a single message (one per submodule, consumer surface, shared utility) — sequential dispatch is a protocol violation. Consumer-surface sweep and Hyrum's-Law review are also mandatory on Deep.
- **At most 3 clarifying questions.** Targets are concrete (file paths or a module name); prefer reading code over interviewing. Lightweight is usually zero questions.
- **HARD GATE on write.** The skill never writes `AUDIT.md` until the user approves the finding set, the scope tier, and the output path. Re-audits on the same target append a `## Re-audit <DATE>` section rather than overwriting — prior findings and resolved-since-last-audit history are preserved.
- **Smart spec-folder reuse.** When the audited module maps to an existing `docs/<DATE>-<slug>/` feature folder, AUDIT.md is written inside it; a new dated folder is created only when no match is found.

Additional invariants for `execute`:

- **Iron Law: no production code without a failing test first.** If production code was written before the test, apply the Delete Rule — delete, write the test, watch it fail, rewrite. **Verification Mode** is the narrow exception for genuinely non-behavior-bearing changes (config, build files, type-only declarations, docs, pure formatting, and mechanical REFACTOR slices like renames / moves / dead-code removal) and requires a captured full-build + full-suite green proof instead of RED.
- **REFACTOR mode: behaviour-preservation is the contract.** The test suite GREEN before a slice must stay GREEN after it. Each non-mechanical refactor slice pins a characterization test on current behaviour first (GREEN baseline captured), applies the refactor, re-runs the same assertions, and verifies they still pass. Addressed findings are back-pointed into AUDIT.md under `## Refactors applied <DATE>` (append-only — the original findings tables are never edited). **Bugs surfaced** by the audit do NOT run through REFACTOR mode — they route to `/diagnosis` first, because a refactor that changes behaviour is a bug fix in disguise and violates the preservation contract.
...
- **Minimal-diff constraint.** Every changed line must trace to a SPEC phase acceptance criterion, a DIAGNOSIS root cause or suggested fix, a selected CODE-REVIEW finding ID (FIX mode), a selected AUDIT finding ID with its named smell (REFACTOR mode), or (RAW mode) a micro-spec acceptance criterion. Lines that don't trace are not written.
- **FIX mode: user-gated findings triage.** In FIX mode, the user decides at Phase 0.5 which CODE-REVIEW findings are addressed this run (default: all P0 + P1) and which are skipped. Skipped findings are recorded in EXECUTION.md's Findings Skipped table with reasons — never silently dropped. Addressed findings are back-pointed into CODE-REVIEW.md under `## Fixes applied <DATE>` (append-only — the original findings tables are never edited). Suggested-Fix disagreements invoke the Confusion Protocol, not silent divergence. **REFACTOR mode** uses the same Phase 0.5 triage against AUDIT.md findings, with the added rule that **Bugs surfaced** items are removed from the refactor pool and recorded as "routed to /diagnosis" — they are never refactored.
...
Copy `skills/teardown/`, `skills/prd/`, `skills/spec/`, `skills/revision/`, `skills/diagnosis/`, `skills/audit/`, `skills/execute/`, `skills/code-review/`, `skills/pr-feedback/`, `skills/commit-push/`, or `skills/solutions/` into your project's `.claude/skills/` directory.
...
```
/eva
/eva I'd like to refactor module src/payments/processor.ts, what do you recommend?
/eva investigate and follow the recommended pipeline to fix bug PROJ-1234
/eva https://github.com/acme/app/pull/742
/eva docs/2026-04-15-checkout-retry/
/eva build a feature for invoice exports — do it
```

Or from a natural description:

> *"What should I use for this Jira card?"*
> *"Help me get started — here's the bug."*
> *"Refactor this module and ship it."*

The skill will:

1. Parse the args — detect URL / path / drive phrase / free-text.
2. Do the minimum classification reading — `gh pr view --json` / `gh issue view --json` / Atlassian MCP `getJiraIssue` with `fields: ["*all"]` + the full `expand` set for Jira, `ls -la` + head + `git status` for local paths, `find | wc -l` for directories, nothing for pure free-text.
3. Map the signal to exactly one intent tag (`understand` / `plan-feature` / `design-feature` / `revise-pair` / `fix-bug` / `refactor-debt` / `implement-approved-plan` / `review-diff` / `handle-pr-feedback` / `ship-diff` / `capture-learnings`).
4. Pick a complexity tier (Lightweight / Standard / Deep) from file count, issue labels, and scope keywords.
5. Compose the canonical pipeline for that intent and trim it based on artifacts already present in the target folder (skip `/prd` if `PRD.md` is approved, skip `/diagnosis` if `DIAGNOSIS.md` is, mark `/solutions` optional on Lightweight).
6. Emit the Routing Plan — under 30 lines, one-line rationale per step, with a real alternative pipeline and the skipped steps called out.
7. If the user asked to DRIVE, dispatch ONLY the first skill via the Skill tool — subsequent steps are always an explicit user call.

**Read-only, pure-router.** `/eva` never writes `.md` files, never edits code, never commits, never posts PR comments, never transitions Jira state, and never drafts artifact content. If you already named a skill (*"/audit src/X"*, *"/spec"*), invoke that skill directly — `/eva` refuses to re-route.
...
```
/audit src/checkout/
/audit src/auth/session.ts
/audit the whole service
```

Or with an explicit scope override:

```
/audit src/checkout/ deep
/audit src/utils/currency.ts lightweight
```

Or from a natural description:

> "Audit the checkout module for structural debt before we add the new payment provider."
> "Survey src/auth/ for smells and rank by refactor priority."
> "Audit the whole backend — I want to know what debt is blocking the next quarter."

The skill will:

1. Resume-check `docs/` for an existing AUDIT.md on the same target; offer to append a `## Re-audit <DATE>` section (recommended), overwrite, or write to `AUDIT-N.md`.
2. Scan `docs/*/` for a related spec folder — if the audited module maps to a known feature, offer smart-reuse of the folder (AUDIT.md sits next to the PRD/SPEC); otherwise derive a new `docs/<DATE>-<slug>/`.
3. Classify scope (Lightweight / Standard / Deep) from file count + LOC + cross-module signals + risk tags (concurrency, auth, payments, migrations, public APIs promote one tier).
4. Ask ≤3 clarifying questions, one at a time, only on genuine ambiguity (target boundaries, scope override, pattern exemplar).
5. **Pre-scan the target**: on Lightweight/Standard, single-pass read; on Deep, dispatch **3-5 parallel `Explore` agents in a single message** — one per submodule, one per primary consumer surface, one per shared utility cluster the target imports. Sequential dispatch is a protocol violation.
6. Pick a pattern exemplar — the best-shaped analogue file inside the target itself — to calibrate "good" relative to the surrounding code. On Deep, also read one test file to calibrate the test-shape baseline before flagging T1-T5.
7. Walk the target with the three-layer smell vocabulary in hand (see `references/smell-playbook.md`) — Fowler smells, no-workaround categories, test anti-patterns. Every finding cites the named smell, file:line, verbatim evidence (5-30 word quote), cost band, and a one-sentence recognition test.
8. (Deep only) Run the consumer-surface sweep — for every exported symbol, grep callers and note any caller that depends on implementation detail. Run the Hyrum's-Law review — what observable details (field order, error message text, status-code choice, timestamp format) may external consumers have started depending on?
9. Separate any live crash / data loss / silent-failure / security-hole finding into the **Bugs surfaced** section with a `/diagnosis` recommendation — these are NOT audit findings and will NOT be addressable via REFACTOR mode.
10. Calibrate severity against the P0/P1/P2/P3 rubric (see `references/scope-tiers.md`). If the P1 count exceeds the tier's ceiling (5/10/20), re-tier downward — everything-is-P1 is a calibration error.
11. Surface deliberately-embraced patterns into the **Suppressed candidates** table with the reason (ADR cited, comment confirmed, convention file named, repo's wider pattern matches) — never silently drop them.
12. Assemble the Handoff table — one row per finding with (ID, severity, named smell, file:line, suggested direction) that `/execute` REFACTOR mode parses directly at Phase 0.5 triage.
13. Emit a self-review checklist (every finding names a smell from the three layers, every finding quotes verbatim evidence, deliberate patterns suppressed not flagged, bugs routed to /diagnosis, clean-pass posture allowed, P1 count within the tier ceiling, Deep fan-out ran in a single message).
14. **HARD GATE** — propose the finding set, the scope tier, and the output path; wait for approval.
15. Write `docs/<DATE>-<slug>/AUDIT.md` (or append `## Re-audit <DATE>`). Close with a one-sentence transition hint — `/execute` in REFACTOR mode on this AUDIT.md to apply selected refactors, or `/diagnosis` on any Bugs surfaced item. Never edits the audited code, never commits, never invokes `/execute` or `/diagnosis`.
...
/execute docs/2026-04-17-checkout-audit/AUDIT.md
```
...
> "/execute apply the refactors from the checkout audit."
> "/execute resolve the P1 smells in the audit."
...
2. Auto-detect the source of truth in priority order: explicit path (`AUDIT.md → REFACTOR`, `CODE-REVIEW.md → FIX`, `SPEC.md → FEATURE`, etc.) → REFACTOR-mode prompt triggers + newest `AUDIT.md` with `status: approved` → FIX-mode prompt triggers + newest `CODE-REVIEW.md` with `status: approved` → `REVISION.md` with `patches_applied: true` → `SPEC.md` with `status: approved` → `DIAGNOSIS.md` with `status: approved` → `PRD.md` alone (warns and suggests `/spec` first) → raw prompt (RAW mode). When a detected primary artifact shares a folder with an approved AUDIT.md (with open findings) or CODE-REVIEW.md (with open findings), the skill offers a one-question mode-switch before advancing.
3. Resume-check for an in-progress `EXECUTION.md` in the target folder; offer to resume or restart.
4. **Findings triage at Phase 0.5** (FIX mode and REFACTOR mode) — parse every finding in the latest findings section of CODE-REVIEW.md / AUDIT.md, subtract any finding already addressed by a prior `## Fixes applied <DATE>` (FIX) or `## Refactors applied <DATE>` (REFACTOR) section, and display a compact P0/P1/P2/P3 summary. In REFACTOR mode, separate **Bugs surfaced** items out of the refactor pool entirely and record them as "routed to /diagnosis" (a refactor that changes behaviour is a bug fix). Ask the user to pick: **A. All P0 + P1 (default)** / **B. P0 + P1 + P2** / **C. Every finding shown** / **D. Custom** (include/skip by ID) / **E. Abort**. Selected findings drive the slice plan; skipped findings are recorded with user-supplied reasons for the EXECUTION.md audit trail and remain open for the next re-review or re-audit.
...
6. Classify scope (Lightweight / Standard / Deep) based on signals (file count, concurrency, SPEC complexity, FIX-mode or REFACTOR-mode selected-finding tally).
7. Ask ≤3 clarifying questions, one at a time, only on genuine ambiguity (definition of done, scope boundaries, test baseline). Usually 0-1 when an artifact is the source of truth — FIX / REFACTOR modes typically ask zero because the findings are the spec.
8. Derive a vertical-slice plan — one slice per SPEC phase in FEATURE mode, one slice (two if defense-in-depth was recommended) in BUG mode, one slice per selected finding (or grouped when findings share file + fingerprint root) in FIX mode, one slice per selected finding (same grouping rule; slice intent names the smell — `Refactor F-3 (P1, Shotgun Surgery): extract currency-conversion module`) in REFACTOR mode, or an internal micro-spec + 1-4 slices in RAW mode.
9. Emit a self-review checklist.
10. **HARD GATE 1** — propose the slice plan, scope tier, branch, output path, commit style, and (FIX / REFACTOR mode) the selected vs skipped findings; wait for approval before touching any file.
11. Execute slice-by-slice. **FEATURE / BUG / FIX / RAW modes**: **RED** (write the failing test, run it, capture literal output) → **GREEN** (write the minimum code to pass) → **REFACTOR** (only while green) → full suite + lint + types → **COMMIT** (Conventional Commits format — FIX mode commits reference the finding ID; `refactor:` prefix only in REFACTOR mode — explicit files, no push, no PR). **REFACTOR mode**: **characterization-tdd** — pin the current behaviour with a GREEN characterization test BEFORE touching the code, apply the refactor, re-run the same test, verify it stays GREEN, run the full suite. Mechanical refactors (pure renames, file moves, dead-code removal, signature-preserving extractions) may use **Verification Mode** — skip the characterization test, capture a full-build + full-suite green proof instead. Non-behavior slices in any mode (config, build files, type-only, docs, pure formatting) also use Verification Mode.
12. On a 3-strike RED failure (FIX / FEATURE / BUG / RAW) or a 3-strike behaviour-regression in REFACTOR (characterization test goes RED after the refactor), STOP coding and invoke the Step-Back protocol — do not attempt a fourth approach; offer to escalate to `/diagnosis`, re-open `/spec`, or ask the user for guidance.
13. Log out-of-scope observations with the **NOTICED BUT NOT TOUCHING** prefix; never act on them mid-execute. In FIX / REFACTOR mode, this applies doubly — tempting adjacent issues surfaced by the code review or the audit that the user did NOT select at triage must be logged, not fixed.
14. Run the integration gate — fresh full test suite, lint, and type-check; audit skipped tests; (BUG mode and FIX mode when practical) capture a regression red-green proof by reverting the fix → test RED → restoring the fix → test GREEN; (REFACTOR mode) capture the behaviour-preservation proof — full suite GREEN before Slice 1 and after the last slice, as literal before/after command output. In FIX mode, at least one red-green proof per addressed P0 finding.
...
16. Write `docs/<DATE>-<slug>/EXECUTION.md` with literal test output for every slice, the integration gate log, acceptance criteria trace, NOTICED-BUT-NOT-TOUCHING list, follow-ups, concerns, the commit table, and (FIX / REFACTOR mode) Findings Addressed + Findings Skipped tables. Append the back-pointer: `## Implemented <DATE>` on SPEC.md (FEATURE), `## Fixed <DATE>` on DIAGNOSIS.md (BUG), `## Fixes applied <DATE>` on CODE-REVIEW.md (FIX), or `## Refactors applied <DATE>` on AUDIT.md (REFACTOR), always with an addressed-vs-skipped breakdown. Never push, never open a PR. Next step for the user in FIX mode: re-run `/code-review` for a `## Re-review <DATE>` delta. Next step in REFACTOR mode: re-run `/audit` for a `## Re-audit <DATE>` section confirming prior findings cleared.
...
## How `eva`, `teardown`, `prd`, `spec`, `revision`, `diagnosis`, `audit`, `execute`, `code-review`, `pr-feedback`, `commit-push`, and `solutions` compose

`eva` sits **above** the pipeline — it reads intent from any input and picks the first skill (and the shape of the rest) for you. Every arrow below can start from `eva` when you want help choosing where to enter.
...
```
  ...
  │       ▲                          (severity-ranked smells from 3-layer       │
  │       │                           vocabulary — Fowler + no-workarounds +    │
  │       │                           test anti-patterns; Bugs surfaced         │
  │       │                           routed out to /diagnosis)                 │
  │       │                                   │                                  │
  │       │                                   ▼                                  │
  │       │          execute (REFACTOR mode)  ──▶  Phase 0.5 triage             │
  │       │                 │                    (user picks which smells;      │
  │       │                 │                     Bugs surfaced filtered out)   │
  │       │                 ▼                                                    │
  │       │         characterization-tdd per slice:                              │
  │       │           pin GREEN baseline ──▶ apply refactor ──▶                 │
  │       │           verify same test still GREEN ──▶ full suite GREEN         │
  │       │         (mechanical refactors use Verification Mode instead)        │
  │       │                 │                                                    │
  │       │                 ▼                                                    │
  │       │         EXECUTION.md + commits                                       │
  │       │         (refactor: <smell> — <title> subjects,                       │
  │       │          behaviour-preservation proof captured in                    │
  │       │          Integration Gate — full suite GREEN before                  │
  │       │          Slice 1 and after the last slice)                           │
  │       │                 │                                                    │
  │       │                 ▼                                                    │
  │       │         AUDIT.md gains  ## Refactors applied <DATE>                  │
  │       │         (addressed + skipped tables with named smells;               │
  │       │          originals untouched; Bugs surfaced still pending)           │
  │       │                 │                                                    │
  │       │                 ▼                                                    │
  │       └────── /audit  ──▶  appends  ## Re-audit <DATE>                       │
  │                           (prior findings cleared, surviving ones,          │
  │                            newly-surfaced debt from the refactor            │
  │                            itself; loop continues until debt is             │
  │                            acceptable or the scope shifts)                  │
  │                                                                              │
  │   Bugs surfaced  ──▶  /diagnosis  (separate turn — never REFACTOR mode,     │
  │                                    because a behaviour fix violates the    │
...
- **AUDIT** answers *what structural debt lives in this stable code, and which piece costs the most* — a severity-ranked inventory of smells (P0 Blocker debt / P1 Major / P2 Minor / P3 Nit) against a three-layer vocabulary (13 Fowler smells + 7 no-workaround categories + 5 test anti-patterns), each with named smell, file:line, verbatim evidence, cost band, and a one-sentence recognition test; Positives the refactor must not regress; a Suppressed-candidates table for deliberately-embraced patterns; a **Bugs surfaced** section that routes live failures to `/diagnosis` (never mixed into refactor findings); architecture observations for cross-cutting shape issues; a scope-calibration footer; and the Handoff table `/execute` REFACTOR mode consumes directly. Pure reporter — never edits code, never commits. The only eva artifact that catalogues debt on a static snapshot without reading git history or profiling at runtime.
- **EXECUTION** answers *what shipped, how, and what's proven* — literal RED and GREEN proofs per vertical slice (characterization baseline GREEN proofs in REFACTOR mode), an integration-gate log with full suite + lint + types, acceptance-criteria trace mapping each SPEC criterion / DIAGNOSIS reproduction test / selected CODE-REVIEW finding / selected AUDIT finding to the slice that satisfies it, NOTICED-BUT-NOT-TOUCHING list, follow-ups, concerns, the Conventional Commits table, (FIX mode) the Findings Addressed + Findings Skipped tables that give the next reviewer a clean audit trail of what was handled vs deferred, and (REFACTOR mode) the Findings Addressed + Findings Skipped tables with named smells plus the Bugs-surfaced-routed-to-/diagnosis block plus the behaviour-preservation proof (full suite GREEN before Slice 1 and after the last slice, as literal before/after output). The only eva artifact produced alongside real code changes.
...
- **`audit` + `execute` REFACTOR mode close the audit → refactor → re-audit loop** — a separate loop from FIX mode, tuned for structural-debt cleanup on stable code. `/audit src/checkout/` produces a severity-ranked AUDIT.md with named smells; `/execute docs/<DATE>-<slug>/AUDIT.md` (or a prompt like `/execute apply the refactors from the checkout audit`) routes to REFACTOR mode, parses the Handoff table, asks the human which findings to address vs skip (default: P0 + P1), filters out any `Bugs surfaced` items (those route to `/diagnosis`, not here), and slices each selected finding as a **characterization-tdd** unit — pin a GREEN characterization test BEFORE the refactor, apply the smell-resolving change, verify the same test stays GREEN, keep the full suite GREEN across every slice (behaviour-preservation is the contract). Mechanical refactors (renames, moves, dead-code) use Verification Mode instead. The skill commits with `refactor: <smell> — <finding title>` subjects referencing the finding ID in the body, records skipped findings in an audit table, and appends `## Refactors applied <DATE>` to AUDIT.md. The user then re-runs `/audit`, which appends `## Re-audit <DATE>` with the delta — cleared findings, surviving ones, and any newly-surfaced ones. Like FIX mode, the loop is **human-paced** through HARD GATE 1 (plan approval) and HARD GATE 2 (green integration gate + behaviour-preservation proof). Unlike FIX mode, the contract is stricter: any slice whose characterization test goes RED after the refactor is a behaviour regression — Step-Back and reassess, never "accept the drift."
