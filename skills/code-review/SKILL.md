---
name: code-review
description: Audit current code changes through a multi-reviewer process covering plan compliance, code quality, conventions, test coverage, and (conditionally) security, performance, and adversarial review. Use when the user wants to review the current diff, audit changes before commit or PR, says "review my code", "check these changes", "audit this diff", or invokes /code-review. Auto-detects diff scope and produces docs/YYYY-MM-DD-slug/CODE-REVIEW.md with severity-grouped findings. Distinct from /audit (stable code) and /pr-feedback (reviewer comments). Pure reporter — never writes or edits code.
