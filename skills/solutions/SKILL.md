---
name: solutions
description: Capture durable learnings from the pipeline that just completed into a single synthesis document future sessions can read before touching the same area of the codebase. Use at the end of any dev or resolve pipeline, when the user says "solutions", "learnings", "capture learnings", "write it up", "wrap up the pipeline", "post-mortem", "retrospective", "lessons learned", "document what we learned", "record the gotchas", "what did we learn", "compound what we did", or invokes /solutions. Produces docs/YYYY-MM-DD-slug/SOLUTIONS.md that distills PRD / SPEC / REVISION / DIAGNOSIS / EXECUTION / CODE-REVIEW in the active spec folder into a reference with Summary, Root Cause (bug pipelines), Approach, Key Decisions as compact mini-ADRs, Gotchas phrased as behaviour invariants, What Didn't Work (3-strike Step-Backs, rejected hypotheses, suppressed findings), Prevention (bug pipelines), and References. Scope-adaptive (Lightweight, Standard, Deep — Deep adds a mandatory Mermaid diagram of the durable mechanism). Inline synthesis only — no subagents, no git-log scan, no tacit-knowledge interview. Smart-reuses the existing spec folder; on re-run, appends "## Post-Release Bug Fix <DATE>" when a DIAGNOSIS is present, otherwise appends "## Re-run <DATE>" — the prior content is preserved as history. Refuses to run when no eva artifact exists in the target folder. Gates writing on explicit user approval. NEVER edits CLAUDE.md, AGENTS.md, or any project instruction file.
