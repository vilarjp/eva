---
name: pr-feedback
description: "Triage incoming PR review comments into verdicts (must-fix / nice-to-have / question / push-back / already-done) with confidence scores, verbatim evidence, and copy-pasteable GitHub replies — hands off to /execute FIX mode for the actual fixes. Use when the user says 'triage this review', 'handle PR feedback', 'address the review', 'I got review comments', pastes a GitHub PR URL or `owner/repo#N`, or invokes /pr-feedback. Produces docs/YYYY-MM-DD-<slug>/PR-FEEDBACK.md. Distinct from /code-review (reads the diff) and /execute (applies fixes). Pure triage — never posts to the PR, never edits source."
