---
name: commit-push
description: Commit the current working changes and push to a remote feature branch with safety gates — always confirms the target branch explicitly with the human (never silently derived, never reused on a protected branch), runs a scope-adaptive pre-commit verification (secrets + protected-branch on Lightweight, + tests + lint + debug-artifact scan on Standard, + scope-vs-diff + test-coverage-for-code-changes with chore-exemption audit on Deep), proposes a logical commit split of up to three commits when concerns cluster by file (human approves the grouping), drafts title+body messages that match the repo's existing commit convention (last 10 commits → commitlint config → Conventional Commits default) with a WHY-focused body and a testing-scenarios block for feat/fix, stages explicit files by name (never `git add -A` or `git add .`), fetches and rebases onto the upstream default before pushing with `-u`, and aborts on non-trivial conflicts. Use when the user says "commit this", "commit and push", "commit it", "save my changes", "push my changes", "push this branch", "ship the commit", "save and push", or invokes /commit-push (with optional free-text hint that may include a branch name or subject-line hint). Refuses protected branches (`main`, `master`, `production`, `prod`, `stable`, `live`, `trunk`, `release*`) — if on one, stops and asks the user to create a feature branch first. Never opens a pull request, never runs `gh pr create`, never `git add -A`, never `--force` (only `--force-with-lease` on a second explicit turn), never `--no-verify`, never commits secrets, never modifies source code (only stages, commits, and pushes what's already in the working tree). Writes no new durable artifact by default; when an adjacent `docs/<DATE>-<slug>/EXECUTION.md` or `DIAGNOSIS.md` is present, appends a compact `## Commits <DATE>` back-pointer listing the SHAs so the feature/fix folder stays auditable.
