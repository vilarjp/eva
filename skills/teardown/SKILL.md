---
name: teardown
description: Reverse-engineer a single file (local path or URL) through a rigorous, read-only analysis pipeline that handles minification, hydrates source maps when available, and produces a clear structural map, data-flow walkthrough, and edge-case inventory. Use when the user wants to understand an unfamiliar bundle, decipher minified production code, analyze a third-party script, dissect a competitor's feature, decode an obfuscated payload, says "reverse engineer", "teardown", "dissect", "decompile", "analyze this bundle", "figure out how X works", "walk me through this file", or invokes /teardown. Produces docs/YYYY-MM-DD-slug/TEARDOWN.md with a summary, structural map, behavior walkthrough, data flow (Mermaid diagram on Standard+), edge cases, external surface, and residual-risk section — plus optional reproducible teardown-sources/ sidecars. Supports full-workflow mode AND focused-slice mode triggered when the invocation includes a focus phrase; re-running on an existing target with a new focus appends a dated focused-slice section rather than creating a new file. Scope-adaptive (Lightweight, Standard, or Deep); URL inputs gated on a one-line permission acknowledgment; writing gated on explicit user approval. Read-only in source code.
