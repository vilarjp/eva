---
name: execute
description: Implement approved plans — features from SPEC/REVISION, bugs from DIAGNOSIS, fixes from CODE-REVIEW, refactors from AUDIT, or a direct human request when no artifact exists. Auto-detects the active spec folder and routes to FEATURE, BUG, FIX, REFACTOR, or RAW mode. Use when the user says "build it", "implement the spec", "ship it", "execute", "write the code", "fix this bug", "apply the fix", "address the review findings", "apply the audit", "refactor the findings", or invokes /execute. Follows red-green-refactor TDD, commits one slice at a time on a non-protected branch, and produces docs/YYYY-MM-DD-slug/EXECUTION.md. Gates first-line-of-code on explicit plan approval.
