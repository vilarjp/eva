---
name: eva
description: "Route any task, question, URL, or file path to the right EVA skill or pipeline — the plugin's routing brain. Returns an ordered Routing Plan across EVA's skills (/teardown, /prd, /spec, /revision, /diagnosis, /audit, /execute, /code-review, /pr-feedback, /commit-push, /solutions) with per-step rationale and scope. Use when the user is unsure which skill to invoke, says 'what should I use for X', 'recommend a pipeline', 'help me get started', pastes a GitHub/Jira link, or invokes /eva. Default RECOMMEND; opt-in DRIVE ('do it', 'go ahead') dispatches only the first skill. Pure router — never writes, never edits, never commits."
